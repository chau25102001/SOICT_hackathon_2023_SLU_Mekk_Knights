...
def _model_average(ms = [], p = []):
    if not p: p = [1.0 / len(ms) for _ in range(len(ms))]
    # res = Model().to(ms[0].get_device())
    ensemble_model = copy.deepcopy(ms[0])

    models = []
    keys = []
    # print(model,type(model))
    for layer_name in ms[0].keys():
        # print(key, model[key])
        layers = []
        for i in range(len(ms)):
            model, weight = ms[i], p[i]
            l = model[layer_name] * weight
            layers.append(l)
        ensemble_layer = sum(layers)
        ensemble_model[layer_name] = ensemble_layer
